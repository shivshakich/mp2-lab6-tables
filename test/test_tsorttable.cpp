...
TEST(TSortTable, can_create_table) {
	...
	ASSERT_NO_THROW(TSortTable t(10));

	TSortTable t1, t2(10);
	TRecord rec1, rec2 = { "name", TPolynom("x+y") };
	ASSERT_NO_THROW(TSortTable t(t1));
	ASSERT_NO_THROW(TSortTable t(t2));
	ASSERT_NO_THROW(TSortTable t(rec1));
	ASSERT_NO_THROW(TSortTable t(rec2));
	ASSERT_NO_THROW(TSortTable(TSortTable(rec2)));
}

TEST(TSortTable, check_default_constructor) {
	int n = 10;
	TSortTable t1, t2(n);
...
TEST(TSortTable, check_record_conversion_constructor) {
	int n = 10;
	string name = "_name";
	TPolynom pol("x+y+z");
	TRecord rec1, rec2 = { name, pol };
	TSortTable t1(rec1), t2(rec2);
